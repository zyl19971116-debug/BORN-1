...
async function main() {
  ...
  const Registry = await ethers.getContractFactory("MemeRegistry");
  ...
  await (await factory.setVotingContract(await voting.getAddress())).wait();
  await (
    await registry.setProtocol(
