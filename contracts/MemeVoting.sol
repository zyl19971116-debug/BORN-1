...
contract MemeVoting is Ownable {
    ...
    function startRound(uint256[] calldata ids,uint256 duration,bytes32 seed) external onlyOwner { require(currentRoundId==0||rounds[currentRoundId].finalized,"Active round");require(ids.length>=2&&duration>0,"Invalid round");uint256 id=++currentRoundId;Round storage r=rounds[id];r.roundId=id;r.startTime=block.timestamp;r.endTime=block.timestamp+duration;r.seed=seed;for(uint i;i<ids.length;i++){r.candidateMemeIds.push(ids[i]);registry.setLive(ids[i],true);}emit RoundStarted(id,r.startTime,r.endTime,ids); }
